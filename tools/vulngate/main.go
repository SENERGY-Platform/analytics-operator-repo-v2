...
// four that are transitive - one of them, golang-jwt/jwt v3, with no fix at all
// because that major line is end of life. Plain govulncheck would therefore fail
// a gate the criterion calls green, with no route to green except weakening it.
//
...
// because a silent green run would hide four known vulnerabilities.
//
// Taken from github.com/SENERGY-Platform/moses (same organisation, Apache-2.0),
// where the same gate criterion produced the same problem. Fixes belong in both
// copies until this lives somewhere shared.
//
...
//
// The version is pinned rather than @latest. This gate blocks the release, and a
// scanner release that renames a field in the JSON stream would turn it red on a
// day nobody touched this repository — the vulnerability database is fetched at
// scan time either way, so pinning the scanner does not stale the findings. Bump
// it deliberately, the way the licence and secret scanners are bumped.
var govulncheckArgs = []string{
	"run", "golang.org/x/vuln/cmd/govulncheck@v1.7.0", "-format", "json", "./...",
}
