...
// like tools/vulngate. Fixes belong in both copies until this lives somewhere
// shared.
//
// It runs the whole suite, so the integration tests in pkg/db need MONGO_TEST_URL
// and REQUIRE_MONGO in the environment to be measured rather than skipped — a run
// without them reports a lower figure for reasons that have nothing to do with the
// change.
//
